...
def suggest_person(vk_bot, offer, user_id, my_keyboard=keys.p_keyboard):
    pprint(offer)
    message = '{} {} \n{}'.format(offer['first_name'], offer['last_name'],
                                  offer['href'])
...
def start_search(user_id, vk_bot, info):
    ...
    while True:
        ...
        if req == "В избранное":
            ...
            loop.run_until_complete(quick_commands.add_fav(user_id=user_id, fav_id=offer[counter]['id']))
            botfuncs.write_msg_wk(user_id, "Пользователь успешно добавлен!", keys.p_keyboard)
            counter += 1
            suggest_person(vk_bot, offer[counter], user_id)
            if counter == len(offer) - 1:
                offset += 20
