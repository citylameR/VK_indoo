...
class Registration:
    ...
    def getname(self):
        self.botfunc.write_msg_wk(self.id, f'Вас зовут {self.info["first_name"]}?', keys.register_keys)
        ...
        while True:
            ...
            self.botfunc.write_msg_wk(self.id, "Я вас не понимаю :(\nДавайте попробуем ещё раз!", keys.register_keys)
            req = self.botfunc.listen()

    def getcity(self):
        self.botfunc.write_msg_wk(self.id,f'Ваш город {self.info["city"]["title"]}?',keys.register_keys)
        req = self.botfunc.listen()
        while True:
            if req == "Поменять":
    ...
    def getage(self):
        if self.info["age"] == None:
            self.botfunc.write_msg(self.id, "Мне не увидеть дату рождения :(\nСколько тебе лет?")
            return self.botfunc.listen()
        self.botfunc.write_msg(self.id, f'Ваш возраст {self.info["age"]}?',keys.register_keys)
        req = self.botfunc.listen()
        while True:
            if req == "Поменять":
    ...
    def getage_max(self):
        self.botfunc.write_msg(self.id, "Теперь введите максимальный возраст для поиска:")
        return self.botfunc.listen()