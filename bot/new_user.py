...
def new_user(user_id, botfunc, vk_bot):
    botfunc.write_msg(user_id, "Добро пожаловать в наш сервис для знакомств!"
    ...
    loop.run_until_complete(quick_commands.add_user(user_id=user_id, first_name=info['first_name'],
                                                    last_name=info['last_name'], city=info['city'], age=info['age'],
                                                    age_min=info['age_min'], age_max=info['age_max'],
                                                    sex=info['sex']))
